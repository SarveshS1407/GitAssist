...
import { TechDebtView } from '../src/ui/views/TechDebtView.js';

describe('TechDebtView Component', () => {
  beforeEach(() => {
  ...
  test('renders empty state when no repository is loaded', () => {
    ...
    const view = new TechDebtView({ repositoryState });
    const el = view.render();
    assert.strictEqual(el.className, 'view-container');
    assert.strictEqual(el.children.length, 2);
  });

  test('renders tech debt card when repository is loaded', () => {
